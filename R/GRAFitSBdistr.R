...
GRAFitSBdistr <- function(image=image,main_source=main_src,model=NULL,bulge_Model=NULL,disk_Model=NULL,segim=segim) {

  src_mdlSB=GRAFitSBprofile(image=image,main_source=main_src,segim=segim,model=model,comp="bd",
                            col = 'green', modelOPlot=TRUE, legend=TRUE, main = "Surface Brightness Profile", plot = FALSE)
  ...
  magplot(mean_src$x,mean_src$yquan[,1], ylim = c(26,18),xlim = c(min(src_mdlSB$rSrc*PIXELSCALE),max(src_mdlSB$rSrc*PIXELSCALE)),
          col='grey',pch=16, xlab = "Distance to center / asec", ylab = expression(mu(mag/asec^2) ))
