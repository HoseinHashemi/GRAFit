GRAFitEllipsePlotMass <- function (Data, modellist, bulgeloc = 1, diskloc = 2, pixscale = 1, 
          FWHM = 1, SBlim = 26, df = 100, raw = FALSE) 
{
  if (missing(Data)) {
  ...
  imageellipse = profitEllipse(Data$image * region, xcen = modellist$sersic$xcen[diskloc], 
  ...
  sigmaellipse = profitEllipse(Data$sigma * region, xcen = modellist$sersic$xcen[diskloc], 
                               ycen = modellist$sersic$ycen[diskloc], ang = modellist$sersic$ang[diskloc], 
                               axrat = modellist$sersic$axrat[diskloc], box = modellist$sersic$box[diskloc])
  ...
  diskellipse = profitEllipse(disk$z, xcen = modellist$sersic$xcen[diskloc], 
                              ycen = modellist$sersic$ycen[diskloc], ang = modellist$sersic$ang[diskloc], 
                              axrat = modellist$sersic$axrat[diskloc], box = modellist$sersic$box[diskloc])
  totalellipse = profitEllipse(total$z, xcen = modellist$sersic$xcen[diskloc], 
                               ycen = modellist$sersic$ycen[diskloc], ang = modellist$sersic$ang[diskloc], 
                               axrat = modellist$sersic$axrat[diskloc], box = modellist$sersic$box[diskloc])
  psfellipse = cbind(seq(0, 10 * FWHM, len = 1000), dnorm(seq(0, 
                                                              10 * FWHM, len = 1000), sd = FWHM/(2 * sqrt(2 * log(2)))))
  ...
  sigmaellipse = cbind(sigmaellipse, imageellipse[, 2] - sigmaellipse[, 
                                                                      2])
  sigmaellipse = cbind(sigmaellipse, imageellipse[, 2] + sigmaellipse[, 
                                                                      2])
  # imageellipse[, 2] = -2.5 * suppressWarnings(log10(imageellipse[, 
  #                                                                2])) + 5 * log10(pixscale) + Data$magzero
  # sigmaellipse[, 2:4] = -2.5 * suppressWarnings(log10(sigmaellipse[, 
  #                                                                  2:4])) + 5 * log10(pixscale) + Data$magzero
  # bulgeellipse[, 2] = -2.5 * suppressWarnings(log10(bulgeellipse[, 
  #                                                                2])) + 5 * log10(pixscale) + Data$magzero
  # diskellipse[, 2] = -2.5 * suppressWarnings(log10(diskellipse[, 
  #                                                              2])) + 5 * log10(pixscale) + Data$magzero
  # totalellipse[, 2] = -2.5 * suppressWarnings(log10(totalellipse[, 
  #                                                                2])) + 5 * log10(pixscale) + Data$magzero
  # psfellipse[, 2] = -2.5 * suppressWarnings(log10(psfellipse[, 
  #                                                            2])) + 5 * log10(pixscale) + Data$magzero
  ...
  if (raw) {
    predict.image = list(x = imageellipse[, 1], y = imageellipse[, 
                                                                 2])
    predict.sigma = list(x = sigmaellipse[, 1], y = sigmaellipse[, 
                                                                 2])
    predict.sigma.lo = list(x = sigmaellipse[, 1], y = sigmaellipse[, 
                                                                    3])
    predict.sigma.hi = list(x = sigmaellipse[, 1], y = sigmaellipse[, 
                                                                    4])
    predict.bulge = list(x = bulgeellipse[, 1], y = bulgeellipse[, 
                                                                 2])
    predict.disk = list(x = diskellipse[, 1], y = diskellipse[, 
                                                              2])
    predict.total = list(x = totalellipse[, 1], y = totalellipse[, 
                                                                 2])
    predict.psf = list(x = psfellipse[, 1], y = psfellipse[, 
                                                           2])
  }
  else {
    smooth.image = smooth.spline(imageellipse, df = df)
    smooth.sigma.mid = smooth.spline(sigmaellipse[, c(1, 
                                                      2)], df = df)
    smooth.sigma.lo = smooth.spline(sigmaellipse[, c(1, 3)], 
                                    df = df)
    smooth.sigma.hi = smooth.spline(sigmaellipse[, c(1, 4)], 
                                    df = df)
    ...
    predict.image = predict(smooth.image, imageellipse[, 
                                                       1])
    predict.sigma.mid = predict(smooth.sigma.mid, imageellipse[, 
                                                               1])
    predict.sigma.lo = predict(smooth.sigma.lo, imageellipse[, 
                                                             1])
    predict.sigma.hi = predict(smooth.sigma.hi, imageellipse[, 
                                                             1])
    predict.bulge = predict(smooth.bulge, imageellipse[, 
                                                       1])
    predict.disk = predict(smooth.disk, imageellipse[, 1])
    predict.total = predict(smooth.total, imageellipse[, 
                                                       1])
  }
  psfellipse[, 2] = psfellipse[, 2] + min(predict.bulge$y) - 
    min(psfellipse[1, 2])
  smooth.total = smooth.spline(totalellipse, df = df)
  ...
  if (pixscale == 1) {
    ...
    ylab1 = expression(Sigma  * "(" * 10 ^8 * M['\u0298']/pix  ^2 * ")")
    ylab2 = expression(Delta * Sigma * "(" * 10 ^8 * M['\u0298']/pix  ^2 * ")" )
  }
  else {
    xlab = "Project Major Axis / asec"
    ylab1 = expression(Sigma  * "(" * 10 ^8 * M['\u0298']/asec  ^2 * ")")
    ylab2 = expression(Delta * Sigma * "(" * 10 ^8 * M['\u0298']/asec  ^2 * ")" )
  }
  magplot(0, 0, type = "n", ylim = c(yhi,SBlim + 1), 
          xlim = c(0, .6), col = "black", xlab = "", ylab = ylab1, grid = T,
          labels = c(F, T))
  polygon(sigma.polygon[, 1], sigma.polygon[, 2], col = hsv(v = 0, 
                                                            alpha = 0.1), border = NA)
  ...
  legend("topright", legend = c("Data", "ProFit Total", "ProFit Bulge", 
                                "ProFit Disk"), lty = 1, col = c("black", "darkgreen", 
                                                                        "red", "blue"), bg = "white")
  magplot(predict.image$x, predict.image$y - predict.total$y, 
          type = "l", xlim = c(0, .6), ylim = c(-0.5, 0.5), col = "darkgreen",
          xlab = xlab, ylab = ylab2, grid = T, labels = c(T, T))
  polygon(sigma.polygon[, 1], sigma.polygon[, 2] - c(predict.total$y, 
                                                     rev(predict.total$y)), col = hsv(v = 0, alpha = 0.1), 
          border = NA)
  abline(h = 0)
  abline(v = FWHM/2, lty = 3)
}