TinyTimACSR_labtop<-function(name, chip, x, y, filter, focus, ebmv=0){
    setwd(input_dir)
    psf_runfile <- paste(input_dir,'/',name,'_run',sep='')
    fileConn<-file(psf_runfile)
    writeLines(paste("/usr/local/tinytim-7.5/tiny1 ",name,"_parameter_file ", "ebmv=",ebmv," << EOF",sep=''), fileConn)
    close(fileConn)
    ...
    Sys.sleep(1)
    system(paste('/usr/local/tinytim-7.5/tiny2 ',input_dir,'/',name,'_parameter_file',sep=''))
    Sys.sleep(3)
    system(paste('/usr/local/tinytim-7.5/tiny3 ',input_dir,'/',name,'_parameter_file',sep=''))
